...
class StowSynMapper:
    ...
    def __init__(
        ...
        *,
        ...
        self.sampling_frame_shuffle = sampling_frame_shuffle
        self.num_classes            = num_classes
        self.use_v1_jittering       = use_v1_jittering
        # fmt: on
        logger = logging.getLogger(__name__)
        mode = "training" if is_train else "inference"
        logger.info(f"[DatasetMapper] Augmentations used in {mode}: {augmentations}")
        # h5 method to support usage on hyak cluster
        self.h5_file                = None
    ...
    def from_config(cls, cfg, is_train: bool = True):
        ...
        sampling_frame_range = cfg.INPUT.SAMPLING_FRAME_RANGE
        sampling_frame_shuffle = cfg.INPUT.SAMPLING_FRAME_SHUFFLE
        use_v1_jittering = "v1jittering" in cfg.INPUT.AUGMENTATIONS
        ...
        ret = {
            ...
            "sampling_frame_shuffle": sampling_frame_shuffle,
            "num_classes": cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES,
            "use_v1_jittering": use_v1_jittering
        }

        return ret

    def read_image(self, dataset_dict, frame_idx, format="BGR"):
        if 'preload_image' in dataset_dict:
            # loaded into memory
            image = dataset_dict['preload_image'][frame_idx]
            if format == "BGR":
                image = image[:, :, ::-1]
            elif format == "RGB": 
                pass
            ...
            return image
        elif dataset_dict['image_root'].endswith('.h5'):
            # load from h5 file
            if self.h5_file is None:
                self.h5_file = h5py.File(dataset_dict['image_root'], 'r')
            fname = dataset_dict['file_names'][frame_idx]
            v, f, type = fname.split("_")
            scene_id = int(v[1:])
            frame_id = int(f[1:])
            image = self.h5_file[f'{type}'][scene_id, frame_id][:, :, :3]
            if format == "BGR":
                image = image[:, :, ::-1]
            elif format == "RGB": 
                pass
            ...
            image = utils.read_image(dataset_dict["frames"][frame_idx]["file_name"], format=self.image_format)
    ...
    def __call__(self, dataset_dict):
        ...
        if self.is_train:
            ...
            selected_idx = np.random.choice(
                np.array(list(range(start_idx, ref_frame)) + list(range(ref_frame+1, end_idx))),
                min(video_length-1, self.sampling_frame_num - 1), replace=False
            )
            selected_idx = selected_idx.tolist() + [ref_frame]
            selected_idx = sorted(selected_idx)
            if self.sampling_frame_shuffle:
        ...
        for frame_idx in selected_idx:
            ...
            # image = utils.read_image(file_names[frame_idx], format=self.image_format)
            image = self.read_image(dataset_dict, frame_idx, format=self.image_format)
            if self.use_v1_jittering:
                image = self.random_color_warp(image)
            utils.check_image_size(dataset_dict, image)
            aug_input = T.AugInput(image)
