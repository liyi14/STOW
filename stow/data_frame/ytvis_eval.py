...
def instances_to_coco_json_video(inputs, outputs):
    ...
    scores_perframe = outputs["pred_scores_perframe"] if "pred_scores_perframe" in outputs else [None] * len(scores)
    labels = outputs["pred_labels"]
    masks = outputs["pred_masks"]

    ytvis_results = []
    for instance_id, (s, ps, l, m) in enumerate(zip(scores, scores_perframe, labels, masks)):
        segms = [
...
def _evaluate_predictions_on_coco(
    ...
    coco_eval.summarize(output_dir=output_dir)
