...
__all__ = ["load_ytvis_json", "register_stow_video_instances", "register_stow_image_instances"]

STOW_CATEGORIES_2022 = [
    {"color": [128, 128, 128], "isthing": 1, "id": 1, "name": "I"},
]

def _get_stow_instances_meta():
    thing_ids = [k["id"] for k in STOW_CATEGORIES_2022 if k["isthing"] == 1]
    thing_colors = [k["color"] for k in STOW_CATEGORIES_2022 if k["isthing"] == 1]
    assert len(thing_ids) == 1, len(thing_ids)
    # Mapping from the incontiguous YTVIS category id to an id in [0, 39]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in STOW_CATEGORIES_2022 if k["isthing"] == 1]
    ret = {
...
def register_stow_video_instances(name, metadata, json_file, image_root):
    """
...
def register_stow_syn_video_instances(name, metadata, json_file, image_root):
    """
    Register a dataset in YTVIS's json annotation format for
    instance tracking.
    ...
    assert isinstance(image_root, (str, os.PathLike)), image_root
    # 1. register a function which returns dicts
    # NOTE just to fill this field, load_ytvis_json is not used
    DatasetCatalog.register(name, lambda: load_ytvis_json(json_file, image_root, name))
...
def register_stow_image_instances(name, metadata, json_file, image_root):
    """
...
def register_stow_videoperframe_instances(name, metadata, json_file, image_root):
    """
    Register a dataset in YTVIS's json annotation format for
    instance tracking.
    ...
    # since they might be useful in evaluation, visualization or logging
    MetadataCatalog.get(name).set(
        json_file=json_file, image_root=image_root, evaluator_type="cocovideo", **metadata
    )

def register_yuxiangds_instances(name, metadata, json_file, image_root):
    """
    Register a dataset in YTVIS's json annotation format for
    instance tracking.
...
def register_stow_tabletop_real_instances(name, metadata, json_file, image_root):
    """
    Register a dataset in YTVIS's json annotation format for
    instance tracking.
...
if __name__ == "__main__":
    ...
    meta = MetadataCatalog.get("stow_bin_real_test")

    json_file = "/storage/AURMR/amazon_real_v1/video_instances.json"
    image_root = "/storage/AURMR/amazon_real_v1"
    dicts = load_ytvis_json(json_file, image_root, dataset_name="stow_bin_real_test")
